...
public class TargetUtil {
    ...
    public List<TargetCrawled> getVaildTargetUrls(){
        ...
        try {
            ...
            for (Iterator<String> iter = allUrls.iterator(); iter.hasNext(); ) {
                ...
                if (tmpDoc.select(".prod-price-container").first() == null) {
                    ...
                    String prodName = tmpDoc.select(".prod-buy-header__title").text();
                    lastTargetList.add(TargetCrawled.builder().prodCode(prodCode).url(targetUrl).prodName(prodName).build());
                }
            }
        }catch (IOException e) {
            e.printStackTrace();
